...
def get_text_words(pdf_path, max_side=1600):
    doc = fitz.open(pdf_path)
    page = doc.load_page(0)
    rect = page.rect
    scale = max_side / max(rect.width, rect.height)
    ...
    for w in words:
        ...
        px1 = int(x1 * scale)
        py1 = int(y1 * scale)
        result.append({"text": txt, "x": px0, "y": py0, "w": px1 - px0, "h": py1 - py0})
    return result
...
def annotate(img, matched_rooms, mode):
    ...
    for room in matched_rooms:
        ...
        if mode == "hvac":
            ...
            if "卫生间" in room["word"] or "bath" in room_type or "shower" in room_type:
...
def main():
    ...
    print("正在从 PDF 提取文字...")
    words = get_text_words(INPUT_PDF, max_side=1600)
    print(f"提取到 {len(words)} 个文字区域")
    matched = []
    seen_types = set()
    for w in words:
    ...
    if not matched:
        ...
        matched_floor = matched_hvac + [
            {"word": "淋浴区", "room_type": "shower", "group": [{"x": img.width//3, "y": 2*img.height//3, "w": 1, "h": 1}]}
        ]
    else:
