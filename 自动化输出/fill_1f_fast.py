...
for i in range(len(names)):
    for j in range(i+1, len(names)):
        if (floods[names[i]] & floods[names[j]]).sum() > 100:
...
# ===== 画图 =====
# 创建颜色网格：每个网格分配一个空间ID
space_id = np.zeros((gh, gw), dtype=np.int32)  # 0=未分配
for idx, (root, members) in enumerate(groups.items()):
    for m in members:
        space_id[floods[m]] = idx + 1

# 把网格放大到图片尺寸
# 每个GRID像素对应 pw/GRID 个PDF单位，对应 iw/pw * pw/GRID = iw/GRID 个图片像素
# 简化：直接把space_id resize到图片尺寸
...
for sid in range(1, len(groups)+1):
    ...
    if sid <= len(palette):
        ...
        r, g, b = (200, 200, 200)
    overlay_arr[mask] = [r, g, b, 100]  # 半透明
...
result.save(f'{out_dir}/step1_final.png', quality=95)
print(f"\n✅ step1_final.png")
doc.close()
